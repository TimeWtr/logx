...
import (
	ex "github.com/TimeWtr/logx/error"
	"sync"
	"sync/atomic"
	"time"
)
...
func NewBuffer(capacity int64) *Buffer {
	...
	b := &Buffer{
		active:  bufferPool.Get().(chan string),
		passive: bufferPool.Get().(chan string),
...
func (b *Buffer) sw() {
	...
	b.active, b.passive = b.passive, bufferPool.Get().(chan string)
	b.size = 0
}
...
func (b *Buffer) Close() {
	b.once.Do(func() {
		...
		bufferPool.Put(b.active)
		bufferPool.Put(b.passive)
	})
}
